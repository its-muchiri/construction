-- construction.co.ke — database schema
-- Shared core tables (planning/00-portfolio/shared-database-schema.md) +
-- platform-specific extension tables (planning/03-construction-co-ke/database-schema.md).
...
CREATE TABLE payments (
    ...
    -- can have several milestone charges. See src/Core/Escrow.php.
...
CREATE TABLE commission_rules (
    id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    platform ENUM('laundry', 'rider', 'construction', 'solar', 'event') NOT NULL DEFAULT 'construction',
...
-- (planning/03-construction-co-ke/database-schema.md)
-- ============================================================
...
CREATE TABLE construction_bookings (
    ...
    booking_type ENUM('equipment_rental', 'labor_contract') NOT NULL,
    category VARCHAR(100) NOT NULL,
    status ENUM('open_for_quotes', 'quote_accepted', 'mobilized', 'in_progress', 'awaiting_final_signoff', 'inspection_window', 'completed', 'cancelled', 'disputed') NOT NULL DEFAULT 'open_for_quotes',
...
CREATE TABLE project_quotes (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (booking_id) REFERENCES construction_bookings(id),
    FOREIGN KEY (provider_id) REFERENCES users(id)
) ENGINE=InnoDB;
...
CREATE TABLE project_milestones (
    ...
    status ENUM('pending', 'provider_requested_signoff', 'customer_confirmed', 'disputed', 'released') NOT NULL DEFAULT 'pending',
    escrow_transaction_id BIGINT UNSIGNED NULL,
...
CREATE TABLE equipment_listings (
    ...
    ownership_kyc_document_id BIGINT UNSIGNED NOT NULL,
    insurance_kyc_document_id BIGINT UNSIGNED NULL,
    status ENUM('active', 'rented', 'maintenance', 'inactive') NOT NULL DEFAULT 'active',
    FOREIGN KEY (provider_id) REFERENCES users(id),
    FOREIGN KEY (ownership_kyc_document_id) REFERENCES kyc_documents(id),
...
CREATE TABLE equipment_condition_reports (
    id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    booking_id BIGINT UNSIGNED NOT NULL,
    equipment_listing_id BIGINT UNSIGNED NOT NULL,
    report_type ENUM('handover', 'return') NOT NULL,
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (booking_id) REFERENCES construction_bookings(id),
    FOREIGN KEY (equipment_listing_id) REFERENCES equipment_listings(id),
...
CREATE TABLE crew_listings (
    ...
    certification_kyc_document_id BIGINT UNSIGNED NULL,
    status ENUM('active', 'engaged', 'inactive') NOT NULL DEFAULT 'active',
    FOREIGN KEY (provider_id) REFERENCES users(id),
    FOREIGN KEY (certification_kyc_document_id) REFERENCES kyc_documents(id)
) ENGINE=InnoDB;

-- disputes.booking_id and reviews.booking_id both point at construction_bookings.id;
